// src/components/PecFormForAgent.tsx
...
type Props = {
  agentId: number;
  /** Facultatif : surcharge la valeur par défaut ("Agent") */
  defaultQualite?: 'Agent' | 'Cadre';
  onCreated: (pecId: number) => void | Promise<void>;
  onCancel: () => void;
};

export const PecFormForAgent: React.FC<Props> = ({
  agentId,
  defaultQualite = 'Agent',
  onCreated,
  onCancel,
}) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const created = await pecService.createForAgent(agentId, form);
      ...
      <div className="w-full max-w-xl bg-white rounded-xl shadow-xl border border-slate-200">
        <div className="flex items-center justify-between px-5 py-4 border-b">
          <h2 className="text-lg font-semibold text-slate-800 flex items-center gap-2">
            ...
            Nouvelle PEC — <span className="text-slate-600">Agent</span>
          </h2>
          <button
            type="button"
            onClick={onCancel}
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              <label className="block text-xs text-slate-500 mb-1">Qualité</label>
              <select
                value={form.qualiteMalade || 'Agent'}
                onChange={(e) =>
                  update('qualiteMalade', e.target.value as 'Agent' | 'Cadre')
                }
                className="w-full px-3 py-2 border rounded-lg"
              >
