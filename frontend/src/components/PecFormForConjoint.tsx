...
type Props = {
  conjointId: number;
  defaultBeneficiaireLabel: string; // affichage (nom complet)
  defaultQualite?: string; // ex: "Conjoint(e) de ..."

  onCreated: (pecId: number) => void | Promise<void>;
  onCancel: () => void;
};

export const PecFormForConjoint: React.FC<Props> = ({
  conjointId,
      ...
      <div className="w-full max-w-xl bg-white rounded-xl shadow-xl border border-slate-200">
        <div className="flex items-center justify-between px-5 py-4 border-b">
          <h2 className="text-lg font-semibold text-slate-800 flex items-center gap-2">
            ...
            Nouvelle PEC — <span className="text-slate-600">{defaultBeneficiaireLabel}</span>
          </h2>
          <button
            type="button"
            onClick={onCancel}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 border rounded-lg"
                placeholder='ex: "Conjoint(e) de X Y"'
              />
            </div>
