// src/components/PecFormForSenateur.tsx
...
type Props = {
  senateurId: number;
  /** Texte affiché à côté du titre (optionnel) */
  defaultBeneficiaireLabel?: string; // ex: "Sénateur: NOM POSTNOM PRÉNOM"
  /** Pré-remplissage "Qualité du malade" (optionnel) */
  defaultQualite?: string; // ex: "Sénateur"

  onCreated: (pecId: number) => void | Promise<void>;
  onCancel: () => void;
};

export const PecFormForSenateur: React.FC<Props> = ({
  senateurId,
  ...
  const [form, setForm] = useState<PecCreatePayload>({
    ...
    remarque: '',
    adresseMalade: '',
    qualiteMalade: defaultQualite || '', // vide => le back mettra "Sénateur"
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const created = await pecService.createForSenateur(senateurId, form);
      ...
      <div className="w-full max-w-xl bg-white rounded-xl shadow-xl border border-slate-200">
        <div className="flex items-center justify-between px-5 py-4 border-b">
          <h2 className="text-lg font-semibold text-slate-800 flex items-center gap-2">
            ...
            Nouvelle PEC
            {defaultBeneficiaireLabel && (
              <span className="text-slate-600">— {defaultBeneficiaireLabel}</span>
            )}
          </h2>
          <button
            type="button"
...
export default PecFormForSenateur;